...
// ==========================================
const GOOGLE_SHEET_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwdntzxByvN73YUv3WPj1B3owXWT1y61b_dKFF9z2GEVYjG237-bUQPvqNUY67CruM/exec";

/**
 * Syncs cell modifications directly to Google Sheets when Booking ID exists
 * @param {string} bookingId - The unique Booking ID
 * @param {string} fieldName - Column or Field name edited
 * @param {any} newValue - New value entered in the cell
 * @param {number} rowIndex - Row index in table/data
 */
async function syncCellEditToGoogleSheet(bookingId, fieldName, newValue, rowIndex) {
  if (!bookingId) return;
  ...
  try {
    // Send updated cell data directly to Google Sheet
    ...
    if (resData.status === "success") {
      console.log(`[Google Sheet Sync] Successfully updated ${fieldName} for Booking ID: ${bookingId}`);
    } else {
      console.warn("[Google Sheet Sync] Response warning:", resData);
    }
  } catch (error) {
 ...
 * Attach this function to table cell input / blur / change events
 */
function handleCellEdit(event, rowIndex, fieldName) {
  const targetCell = event.target;
  const newValue = targetCell.isContentEditable ? targetCell.innerText.trim() : targetCell.value;

  // Retrieve current row object from internal state
  const rowData = bookingsData[rowIndex];
  if (!rowData) return;

  // Check if Booking ID has been generated for this row
  const hasBookingId = rowData.bookingId && rowData.bookingId !== "" && rowData.bookingId !== "N/A";

  // Update in-memory state so the edit stays persistent
  rowData[fieldName] = newValue;

  if (hasBookingId) {
    // 1. Sync directly to Google Sheet
    syncCellEditToGoogleSheet(rowData.bookingId, fieldName, newValue, rowIndex);

    // 2. Keep local cache synchronized with edited version (prevents reverting)
    saveToLocalCache();
  } else {
    // If Booking ID is not yet generated, save to local storage as draft
    saveToLocalCache();
  }
}

/**
 * Save current state to local cache (preserves edited version until wipeout)
 */
function saveToLocalCache() {
  try {
    localStorage.setItem("app_bookings_data", JSON.stringify(bookingsData));
  } catch (e) {
...
/**
 * Data Wipe Out Handler
 * Clears memory, local storage, and resets UI
 */
function wipeOutData() {
  if (confirm("Are you sure you want to wipe out all local data?")) {
    bookingsData = [];
    localStorage.removeItem("app_bookings_data");
    renderTable(); // Refresh UI table display
    alert("Data wiped out successfully!");
  }
}
